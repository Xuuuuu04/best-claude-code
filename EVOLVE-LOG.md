...
- 新 Rule 预期触发频率: 每次修改 hook 脚本都应该被 code-reviewer（hook 合规维度）+ security-auditor（安全维度）参考
...
- [NEW] `hooks/review-gate.sh`（70 行）：读 `subagent-events.jsonl` 比对 implementer 数 vs code-reviewer 数，差值>0 注入 `[REVIEW-PENDING]`
...
- [MOD] `agents/tech-researcher.md`：description 扩展远程诊断职责；预加载 `remote-diag-protocol` skill
...
- iOS/Android references 是否帮助 implementer-mobile 减少返工
...
- `creative-direction` — "取名 / slogan / 品牌调性 / 文案方向"
