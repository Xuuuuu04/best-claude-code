...
## 2026-05-14 · v5.3（偏移/幻觉防御强化）

### 触发

用户主动发起，分析 Subagent 体系中最大偏移来源并系统性修复：跨阶段理解漂移、审查者正向偏见、隐式假设不显式化、发现无行号证据、授权范围核对缺失。

### 已执行的变更

#### 变更 1：impl-report 强制段落（artifact-protocol.md）

在 impl-report artifact 结构中新增两个 `severity=blocker` 强制段落：
- `## 授权范围对账`：表格对照每个修改文件与 scope-lock 白名单的对应关系
- `## 输入假设`：显式列出所有未在 scope-lock/requirements 中明确说明的推断

缺失任意一项视为审查 [严重] 发现。

#### 变更 2：阶段意图锚点（output-styles/legion-dispatch.md + dispatch-table.md）

每次阶段跃迁（需求→架构、架构→范围、范围→实现、实现→审查）前，调度器必须重读 `ticket.intent` 并在 downstream 指令中以「**任务原始意图**：{intent}」标注一行，防止跨阶段理解漂移。

#### 变更 3：对抗性审查帧（output-styles/legion-dispatch.md + dispatch-table.md）

派遣任何审查 Agent 时，指令必须以"发现问题"而非"验证实现"为框架：
- 正确：「找出 impl-report 和 diff 中的 bug、scope 越界、契约违背」
- 禁止：「验证实现是否正确」「检查是否符合要求」

正向帧激活确认偏见，是审查深度下降的主因。

#### 变更 4：证据引用强制（skills/code-review-protocol/SKILL.md）

新增「证据引用（必查）」checklist 段：
- 所有 [严重] / [一般] 发现必须附 `{file}:{line}` 行号引用
- 无行号证据的发现自动降级为 [轻微]，不得作为驳回依据

#### 变更 5：v5.2 同义词信号补充（前次提交 35de636，本条补录）

14 个 Agent description 新增中文同义词触发信号，覆盖：华为昇腾、仓颉语言、机器学习/LLM/fine-tune、桌面应用(.exe/.dmg)、移动端(iOS/Android/Flutter)、小程序(抖音/支付宝)、多媒体(MG动画/短视频)、就业教练(跳槽/被裁)、接单(私活/兼职)、数据库设计/ER图、design tokens/暗色模式、创意策划(取名/起名)、客户整理(微信截图/甲方邮件)、学术写作(期刊/LaTeX润色)。

### 跟踪指标

- Agents：39（不变）
- Skills：63（code-review-protocol 新增 1 checklist 段）
- Rules：142→143（artifact-protocol 新增 impl-report-mandatory subsection）
- Hooks：不变

### 预期效益

减少审查 Agent 因正向帧浅层通过、实现 Agent 因隐式假设漂移造成的返工轮次；impl-report 对账段让审查者无需自行 grep 范围核对。

---

## 2026-05-14 · v5.1（成本优化）

### 触发

用户主动发起，原因：模型调用成本过高，需要在保障核心门控质量的前提下系统性降低开销。

### 已执行的变更

#### Phase 1：10 个非核心 Agent opus→sonnet（用户预先手动完成，本次提交）

受影响 Agent：仓颉语言开发专家、代码库研究员、华为昇腾开发专家、多媒体内容生成师、就业教练、接单报价师、机器学习工程师、资深数据库工程师、高级后端工程师、高级运维工程师

**决策依据**：这 10 个 Agent 均为实现/运维/专项域角色，非最终裁决节点；Sonnet 在有明确 scope-lock 和 Skill 加持的情况下输出质量可接受；实测成本节省约 25%。

#### Phase 2：5 个学术专项 Agent opus→sonnet

受影响 Agent：学术论文写作专家、顶会顶刊审稿专家、论文数字审计员、引用审计员、定理证明审计员

**决策依据**：学术流水线有 4 层内置审核环（写作→审稿→数字/引用/定理审计），单 Agent 降级风险被流水线吸收；日常开发不触发该流水线，绝对成本极低。

#### Phase 3：6 个高 maxTurns Agent 削减上限

| Agent | 旧值 | 新值 | 理由 |
|-------|------|------|------|
| 学术论文写作专家 | 200 | 120 | 单轮超 120 转极少见 |
| 顶会顶刊审稿专家 | 150 | 80 | 只读分析不需要 150 转 |
| 高级代码审查师 | 160 | 100 | 超 100 转通常是 scope 问题 |
| 高级安全审计师 | 160 | 100 | 同上 |
| 资深系统架构师 | 130 | 80 | 架构决策应快速收敛 |
| 资深范围规划师 | 100 | 60 | scope-lock 生产不需要 100 转 |

#### Phase 4：修复文档数字漂移

- README 徽章：Skills 58→63，Rules 53→142
- Mermaid 图和文本：同步更新

#### Phase 5：Clarification Gate 降噪

**问题**：46 次 hard-block 全被用户 bypass（100% 无效率），doctor 分析为 gate 偏严。

**根因**：`报错/日志` 关键词无条件触发 `MISSING_ASSET=1` → hard-block，但用户在 prompt 正文中已内联提供错误信息。

**修复**：对"报错/日志"类触发加 prompt 长度判断：
- `LEN < 150`：仍 hard-block（短 prompt 确实缺信息）
- `LEN >= 150`：降为 soft 注入 UNDERSTANDING-CHECK（长 prompt 通常已含内联信息）

截图/图片/设计稿类保持原有 hard-block 逻辑不变。

### 不做的事

- ❌ 高级代码审查师、高级安全审计师、质量总监、资深需求分析师、高级需求审查师、资深系统架构师、资深范围规划师、高级架构审查师 —— 保留 opus（核心门控，一次失误成本 >> 模型费用）
- ❌ Rules 大规模清理（需要数据驱动，当前 142 条基本有效）
- ❌ Claude Code 工作流与提示词设计大师 —— 保留 opus（元治理改动影响全局）

### 跟踪指标

- Agents：39（模型分布：sonnet 26 / opus 13）
- Skills：63（不变）
- Rules：142（不变，文档已对齐）
- Hooks：21 主 + 3 `_lib`（不变）

### 预期效益

日常开发任务成本节省约 30-35%；如有大批量学术任务另节省 40%。

### 回退方式

```bash
git revert <本次 commit hash>
# maxTurns 回调：直接修改对应 agents/*.md 的 maxTurns 字段
```
