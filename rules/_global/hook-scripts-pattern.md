...
## 8. Hook Profile 运行时开关

所有 hook 都通过 `hooks/_lib/run-with-logging.sh` 包装执行。该 wrapper 在调用真 hook 前会 source `hooks/_lib/hook-flags.sh`，根据两个环境变量决定是否执行：
...
每个 hook 的最低 profile 在 `hooks/_lib/hook-flags.sh` 的 `_HOOK_MIN_PROFILE` 数组中登记。`minimal` 只放行生命周期必需（`session-start` / `pre-compact` / `post-compact` / `subagent-start-mark`），`standard` 加入审计/安全/质量（`scope-lock-guard` / `artifact-write-guard` / `post-edit-lint` / `subagent-stop-log` / `instructions-audit`）。

### 新增 hook 的登记流程

1. 在 `hooks/` 中新增 `my-hook.sh`，遵守本规范第 1–7 条
2. 在 `hooks/_lib/hook-flags.sh` 的 `_HOOK_MIN_PROFILE` 数组增加 `"my-hook:minimal"`（或 `standard`/`strict`）
3. 在 `settings.json` 注册事件 → 仍然通过 `run-with-logging.sh` 调用
4. 跑 `bash bin/test-hook-flags.sh` 确认登记数 ≥ 9（自动断言）
5. 跑 `bash bin/doctor.sh`，第 14 节会提示 hooks/ 脚本数与登记数是否一致

**不要**在 hook 内部重复 profile 判断——wrapper 已集中处理。

### 何时关 hook

- `CLAUDE_HOOK_PROFILE=minimal`：调试 hook 本身、compaction 流程调优、或发现 hook 异常耗时想临时关闭审计链
- `CLAUDE_DISABLED_HOOKS=post-edit-lint`：当某个 hook 在当前项目误报且来不及修时

---

## 审查要点

`code-reviewer` 与 `security-auditor` 在审查 hook 脚本修改时，除了通用代码审查，还要对照本规范：
...
- [ ] 新 hook 在 `hooks/_lib/hook-flags.sh` 登记了最低 profile，且通过 `run-with-logging.sh` 包装
