<!--
  ...
  最近升级：2026-05-08（v5.2）
  - 全环节对抗协议：任何 A→B 审查环节默认 until_pass（需求↔需求审查、架构↔架构审查、实现↔代码审查、安全↔安全审计、文档↔内容审查、调研↔调研审查、创意↔内容审查、多媒体↔内容审查）
  - 上下文传递协议强化：长文件/多 bug/图片处理 + 完整性校验
  - NEEDS_USER 处理规则强化：任何 Agent 返回 NEEDS_USER → 主会话必须 AskUserQuestion
  - 新增 PermissionRequest hook：自动批准 .claude/ 目录编辑
  - redeliberation-protocol 降级为代码实现迭代参考模板
  - 39 个 Agent 新增上下文获取协议标准化段落
  - 7 个审查 Agent 新增审查深度标准
  - 版本 v5.0→v5.2

  历史成果：
  - 58 Skills / 53 Rules / 39 Agents / 21 Hooks (+3 _lib)
...
best-claude-code 是公开项目名；Agent Legion 是内部系统名。它是 Claude Code 多 Agent 协作调度系统：39 个专职 Subagent + 58 个 Skill + 53 条 Rule + 21 个 Hook + Router 组成分层门控流水线，从需求分析推进到最终交付。支持全环节对抗迭代、Agent Teams 并行协作（实验性）、Channels 外部事件触发、Routines 定时任务。
...
全环节对抗为默认模式。任何 A→B 审查环节（需求↔需求审查、架构↔架构审查、实现↔代码审查、安全↔安全审计、文档↔内容审查、调研↔调研审查、创意↔内容审查、多媒体↔内容审查）默认 until_pass，不因轮数收工。审查返回 REJECT → 自动触发迭代 → PASS 或穷尽升级。只有用户裁决、权限限制、不可逆动作或工具硬失败才进入 `needs_user/blocked`。

真实业务改动默认 `quality_strategy=adversarial-default`：实现者之外至少需要独立审查、测试或等价证据。用户要求"全面 / 多轮 / 反复 / 对抗 / 质量提高 / 客户不满"时，主动提高门控严度。上线、生产、客户交付、schema、认证/支付/敏感数据默认走 `full`。

迭代 `max_rounds=3`（每对 A→B 独立计数），穷尽后升级给 项目管理师 做根因分析，不直接上报用户。质量证据闭合后不要静默 `done`，先设置 `final_confirmation=asked`、`phase=needs_user`，问用户接受当前结果还是继续深挖。若 ticket 已处于 `phase=needs_user` 且 `final_confirmation=required/asked`，用户下一条回复必须先归类为 `accepted` / `continue_requested` / `specified_check` 并写回 ticket。
