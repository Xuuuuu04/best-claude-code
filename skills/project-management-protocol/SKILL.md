...
description: 项目管理协议。为 项目管理师 提供单跳调度、返工升级、阻塞判定和任务状态机的方法。
when_to_use: 仅当 项目管理师 Agent 在做"下一跳判断 / 返工升级 / 阻塞处理 / 多阶段任务推进"时加载。单一 Agent 自身工作流不应触发。
---

<skill name="project-management-protocol">

<overview>
为 项目管理师 提供任务状态机、单跳调度原则、返工升级阈值、用户拍板触发条件和最小调度输出字段。
</overview>
