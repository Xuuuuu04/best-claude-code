...
<skill>
  <overview>
    <summary>tech-researcher 调研"远程/云端"时的标准作业流程。</summary>
    <scope>
  ...
  <workflow>
    ...
    <phase n="2" name="命令白名单（只读）">
      <description>以下命令在 tech-researcher 内允许直接执行。只读，不改变远程状态：</description>
    ...
    <phase n="3" name="命令黑名单（绝不自动执行）">
      <description>以下命令必须返回主会话要求用户确认，或转交 devops：</description>
      <blacklist>
        ...
        <item>生产部署 / 发版 / 回滚：调 devops Agent 并经用户明确确认</item>
      </blacklist>
    </phase>
...
**判断**：服务健康 / 5xx / 超时 / 拒连 / 需要更多数据
  ...
  <checklist>
    ...
    <agent-boundaries>
      <boundary agent="devops" role="真正执行远程写操作（部署/回滚/配置变更）"/>
      <boundary agent="security-auditor" role="审查远程操作的授权与最小权限"/>
      <boundary agent="tech-researcher" role="只读，负责还原事实 + 给出假设"/>
    </agent-boundaries>
  </checklist>
