...
1. 明确集成目标：外部系统、用户任务、只读/写入能力、权限边界。
2. 设计工具：名称、描述、输入 schema、输出结构、错误类型、幂等性。
3. 鉴权：凭据来源、最小权限、过期/撤销、日志脱敏。
4. 实现：参数验证、超时、重试、速率限制、结构化错误。
5. 测试：schema 校验、happy path、权限不足、外部超时、无效输入。
6. 文档：安装、配置、示例调用、安全注意事项。

## 约束

- 工具描述不能诱导模型越权。
- 不把 secret 放入工具输出或日志。
- 写操作必须可审计，危险操作需要确认或 dry-run。

## 支持文件

- `references/tool-design-template.md`：tool design template。
