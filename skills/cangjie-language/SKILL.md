...
<knowledge type="language-reference" id="type-system">
### 类型系统
...
<knowledge type="language-reference" id="concurrency">
### 并发
- M:N 用户态协程, `spawn {}` → `Future<T>`, `Atomic<T>` `Mutex<T>` `synchronized` `Condition`
</knowledge>

<knowledge type="language-reference" id="c-interop">
### C 互操作
- `foreign func` / `@C` / `CPointer<T>` `CString` `VArray<T>` `CFunc` / `unsafe` 块包裹
</knowledge>

<knowledge type="language-reference" id="compilation">
### 编译
- `cjc` (优化/LTO/PGO/交叉编译) · `cjpm` 包管理
</knowledge>

<index id="doc-index" base="references/">
...
<resources id="knowledge-digest" format="structured-summary" size="400-lines">
### 知识摘要
