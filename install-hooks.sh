#!/bin/bash
# install-hooks.sh — 把 BCC 的 6 个事件 hook 注册进 ~/.claude/settings.json
#
...
# 1. 6 个 hook 脚本必须存在,并确保可执行
for s in session-start precompact posttooluse-guard posttoolusefailure stop-progress-gate userpromptsubmit-router; do
  f="${HOOKS_DIR}/${s}.sh"
...
echo "✓ 已把 6 个事件 hook 注册进 $SETTINGS(路径基于 $HOME)"
