#!/bin/bash
# install-hooks.sh — 把 BCC 的 7 个事件 hook 注册进 ~/.claude/settings.json
#
# 为什么需要它:settings.json 被 .gitignore(含 API keys),hook 注册随之不进 git。
# 搬机器 / 重装后 hook 会全部丢失且不自知。这个脚本进 git,跑一次即恢复注册。
#
# 幂等:只覆盖 hooks 段里这 7 个 key,保留 settings.json 其他字段和 hooks 里的自定义项。
set -euo pipefail
...
# 1. 7 个 hook 脚本必须存在,并确保可执行
for s in session-start precompact pretooluse-guard posttooluse-guard posttoolusefailure stop-progress-gate userpromptsubmit-router; do
  f="${HOOKS_DIR}/${s}.sh"
...
HOOKS_JSON=$(jq -n --arg h "$HOOKS_DIR" '{
  ...
  UserPromptSubmit:   [ { matcher: "",                                       hooks: [ { type: "command", command: ($h + "/userpromptsubmit-router.sh") } ] } ],
  PreToolUse:         [ { matcher: "Bash|Read|Edit|Write|MultiEdit|NotebookEdit", hooks: [ { type: "command", command: ($h + "/pretooluse-guard.sh") } ] } ],
...
# 4. merge:保留 hooks 里已有的其他 key,只覆盖这 7 个;mktemp + mv 原子写
TMP=$(mktemp "${SETTINGS}.XXXXXX")
jq --argjson hooks "$HOOKS_JSON" '.hooks = (.hooks // {}) + $hooks' "$SETTINGS" > "$TMP" && mv "$TMP" "$SETTINGS"

echo "✓ 已把 7 个事件 hook 注册进 $SETTINGS(路径基于 $HOME)"
